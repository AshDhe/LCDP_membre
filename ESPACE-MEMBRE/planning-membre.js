(() => {
  ...
  async function initialiserPage() {
    ...
    try {
      ...
      etatMembre = etatMembreDepuisCookie();
      afficherEtatMembre(etatMembre);

      try {
        const etatCharge = await chargerEtatMembre();
        if (etatCharge) {
          etatMembre = etatCharge;
          afficherEtatMembre(etatMembre);
        }
      } catch (errorEtatMembre) {
        console.error("Erreur statut membre planning :", errorEtatMembre);
        afficherEtatMembre(etatMembre);
      }
  ...
  function etatMembreDepuisCookie() {
    return {
      abonne: membreAbonne(),
  ...
  function initialiserBoutonNouvelleDate() {
    const boutons = Array.from(
      document.querySelectorAll("#bouton-nouvelle-date-planning, [data-action='nouvelle-date-planning']")
    );

    boutons.forEach((bouton) => {
      bouton.href = PAGE_RESERVER_MEMBRE;
      bouton.addEventListener("click", gererClicNouvelleDatePlanning);
    });
  }

  function gererClicNouvelleDatePlanning(event) {
  ...
  function actualiserBoutonFiltrePlanning(bouton) {
    ...
    const afficheRetourAvenir = etat.filtre === "passe";

    bouton.textContent = afficheRetourAvenir ? "À venir" : "Passé";
    bouton.classList.toggle("lcdp-button-filtre-planning-actif", afficheRetourAvenir);
  }
  ...
  async function initialiserListeReservations() {
    ...
    const heading = slot.querySelector(".lcdp-box-liste-card__heading");
    const titre = slot.querySelector("[data-lcdp-liste-card-title]");

    if (heading) {
      heading.classList.add("lcdp-box-liste-card__heading--action");
    }
    ...
    if (titre) {
      ...
      bouton.className = "lcdp-button lcdp-button-primary lcdp-button-planning-nouvelle-date";
      bouton.href = PAGE_RESERVER_MEMBRE;
      bouton.dataset.action = "nouvelle-date-planning";
      bouton.textContent = "Réserver une nouvelle date";
