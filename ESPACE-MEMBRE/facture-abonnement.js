(() => {
  ...
  const SOURCE_PAGE = "facture-abonnement";
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initialiserPageFacture);
  } else {
    initialiserPageFacture();
  }

  async function initialiserPageFacture() {
    ...
    if (!orderid) {
      afficherMessage(racine, "Numéro de commande manquant.");
      return;
    }

    if (!ENDPOINT_FACTUPAIEMENT) {
      afficherMessage(racine, "Le service facturation n’est pas configuré.");
      return;
    }

    try {
      const facture = await chargerFacture(orderid);
      await afficherFactureA4(racine, facture);
  ...
  async function chargerFacture(orderid) {
  ...
  async function afficherFactureA4(racine, facture) {
    racine.innerHTML = "";
    ...
    if (!box) {
      throw new Error("Structure facture A4 incomplète.");
    }
